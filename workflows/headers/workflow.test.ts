...
describe("checkCache", () => {
  it("returns cache miss when domain does not exist", async () => {
    const { checkCache } = await import("./workflow");
    const result = await checkCache("nonexistent.test");
  ...
  it("returns cache miss when domain exists but has no headers", async () => {
    const { upsertDomain } = await import("@/lib/db/repos/domains");
    await upsertDomain({
      name: "nocache.test",
      tld: "test",
      unicodeName: "nocache.test",
    });

    const { checkCache } = await import("./workflow");
    const result = await checkCache("nocache.test");
  ...
  it("returns cache hit when domain has non-expired headers", async () => {
    ...
    await upsertDomain({
      name: "cached.test",
      tld: "test",
      unicodeName: "cached.test",
    });

    const domain = await findDomainByName("cached.test");
    ...
    const { checkCache } = await import("./workflow");
    const result = await checkCache("cached.test");
  ...
  it("returns cache miss when headers are expired", async () => {
    const { upsertDomain, findDomainByName } = await import(
      "@/lib/db/repos/domains"
    );
    await upsertDomain({
      name: "expired.test",
      tld: "test",
      unicodeName: "expired.test",
    });

    const domain = await findDomainByName("expired.test");
    expect(domain).toBeTruthy();
    ...
    const { checkCache } = await import("./workflow");
    const result = await checkCache("expired.test");
