...
export type TrackingRouterDeps = {
  generateVerificationToken?: () => string;
  startAutoVerify: (input: { trackedDomainId: string }) => Promise<void>;
  startInitializeSnapshot: (input: { trackedDomainId: string; domainId: string }) => Promise<void>;
...
function defaultGenerateVerificationToken(): string {
...
export function createTrackingRouter(deps: TrackingRouterDeps) {
  const generateVerificationToken =
    deps.generateVerificationToken ?? defaultGenerateVerificationToken;
  ...
  return createTRPCRouter({
    ...
    verifyDomain: protectedProcedure
      ...
      .input(
        z.object({
          trackedDomainId: z.string().uuid(),
          method: z.enum(["dns_txt", "html_file", "meta_tag"]).optional(),
        }),
      )
      .mutation(async ({ ctx, input }) => {
