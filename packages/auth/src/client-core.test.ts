...
describe("auth client core", () => {
  it("normalizes optional base URLs without inventing a default", () => {
    expect(normalizeAuthBaseURL(undefined)).toBeUndefined();
    expect(normalizeAuthBaseURL("")).toBeUndefined();
    expect(normalizeAuthBaseURL("  https://domainstack.io///  ")).toBe("https://domainstack.io");
  });
