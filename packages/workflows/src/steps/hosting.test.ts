...
describe("detectAndResolveProvidersStep", () => {
  ...
  it("detects Vercel hosting from x-vercel-id header", async () => {
    ...
    const { detectAndResolveProvidersStep } = await import("./hosting");
...
describe("lookupGeoIpStep", () => {
  it("parses iplocate.io response and derives owner and domain", async () => {
    // 1.1.1.1 is mocked in ../test/msw.ts with Cloudflare data
    const { lookupGeoIpStep } = await import("./hosting");
    const res = await lookupGeoIpStep("1.1.1.1");
  ...
  it("returns null when the lookup fails", async () => {
    ...
    const { lookupGeoIpStep } = await import("./hosting");
    const res = await lookupGeoIpStep("1.2.3.4");
    expect(res).toBeNull();
  });
});
