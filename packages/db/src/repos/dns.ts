...
export function createDnsRepo(db: DbClient) {
  return {
    async replaceDns(params: UpsertDnsParams) {
      ...
      await db.transaction(async (tx) => {
        ...
        if (idsToDelete.length > 0) {
          await tx
            .delete(dnsRecords)
            .where(inArray(dnsRecords.id, idsToDelete));
        }
...
export type DnsRepo = ReturnType<typeof createDnsRepo>;
