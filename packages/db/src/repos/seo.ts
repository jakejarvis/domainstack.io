...
export function createSeoRepo(db: DbClient) {
  return {
    async upsertSeo(params: SeoInsert) {
     ...
     */
    async getCachedSeo(domain: string): Promise<CacheResult<SeoResponse>> {
...
export type SeoRepo = ReturnType<typeof createSeoRepo>;
