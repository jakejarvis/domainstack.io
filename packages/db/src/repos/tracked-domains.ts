...
export function createTrackedDomainsRepo(db: DbClient) {
  /**
  ...
  > {
    ...
    for (const groups of recordsByDomain.values()) {
      groups.hosting = deduplicateDnsRecordsByValue(groups.hosting).sort(
  ...
  async function queryTrackedDomainsWithDetails(
    ...
    const { includeDnsRecords = true, includeRegistrarDetails = true } =
      options;
  ...
  return {
     ...
     */
    async createTrackedDomain(params: CreateTrackedDomainParams) {
      const { userId, domainId, verificationToken, verificationMethod } =
        params;
     ...
     */
    async createTrackedDomainWithLimitCheck(
     ...
     */
    async findTrackedDomain(userId: string, domainId: string) {
      const rows = await db
        .select()
        .from(userTrackedDomains)
        .where(
     ...
     */
    async findTrackedDomainById(id: string) {
      const rows = await db
        .select()
        .from(userTrackedDomains)
     ...
     */
    async findTrackedDomainWithDomainName(
      id: string,
     ...
     */
    async getTrackedDomainsForUser(
      userId: string,
     ...
     */
    async countTrackedDomainsForUser(
      userId: string,
      includeArchived = false,
    ): Promise<number> {
      const whereCondition = includeArchived
     ...
     */
    async countActiveTrackedDomainsForUser(userId: string): Promise<number> {
      return this.countTrackedDomainsForUser(userId, false);
    },

    /**
     * Count archived tracked domains for a user.
     */
    async countArchivedTrackedDomainsForUser(userId: string): Promise<number> {
      const whereCondition = and(
        eq(userTrackedDomains.userId, userId),
     ...
     */
    async countTrackedDomainsByStatus(
      userId: string,
     ...
     */
    async verifyTrackedDomain(id: string, method: VerificationMethod) {
      const now = new Date();
     ...
     */
    async setDomainMuted(id: string, muted: boolean) {
      const updated = await db
        .update(userTrackedDomains)
        .set({ muted })
        .where(eq(userTrackedDomains.id, id))
     ...
     */
    async deleteTrackedDomain(id: string): Promise<boolean> {
     ...
     */
    async getVerifiedTrackedDomainIds(): Promise<string[]> {
      const rows = await db
        .select({ id: userTrackedDomains.id })
        .from(userTrackedDomains)
        .where(
     ...
     */
    async getTrackedDomainForNotification(
      trackedDomainId: string,
     ...
     */
    async getTrackedDomainForReverification(
      trackedDomainId: string,
     ...
     */
    async markVerificationSuccessful(id: string) {
      const updated = await db
        .update(userTrackedDomains)
        .set({
          verificationStatus: "verified",
     ...
     */
    async markVerificationFailing(id: string) {
      const updated = await db
        .update(userTrackedDomains)
        .set({
     ...
     */
    async revokeVerification(id: string) {
      const updated = await db
        .update(userTrackedDomains)
        .set({
          verified: false,
     ...
     */
    async archiveTrackedDomain(id: string) {
      const updated = await db
        .update(userTrackedDomains)
     ...
     */
    async unarchiveTrackedDomain(id: string) {
      const updated = await db
        .update(userTrackedDomains)
        .set({ archivedAt: null })
        .where(eq(userTrackedDomains.id, id))
     ...
     */
    async unarchiveTrackedDomainWithLimitCheck(
      id: string,
      userId: string,
      maxDomains: number,
     ...
     */
    async archiveOldestActiveDomains(
      userId: string,
     ...
     */
    async bulkArchiveTrackedDomains(
      userId: string,
     ...
     */
    async bulkRemoveTrackedDomains(
      userId: string,
      trackedDomainIds: string[],
     ...
     */
    async getArchivedDomainsForUser(
      userId: string,
    ): Promise<TrackedDomainWithDetails[]> {
      const whereCondition = and(
        eq(userTrackedDomains.userId, userId),
     ...
     */
    async getStaleUnverifiedDomains(cutoffDate: Date) {
      const rows = await db
        .select({
          id: userTrackedDomains.id,
          userId: userTrackedDomains.userId,
     ...
     */
    async deleteStaleUnverifiedDomains(ids: string[]): Promise<number> {
...
export type TrackedDomainsRepo = ReturnType<typeof createTrackedDomainsRepo>;
