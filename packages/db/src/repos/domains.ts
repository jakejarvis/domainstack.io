...
export function createDomainsRepo(db: DbClient) {
  return {
    /**
     ...
     */
    async upsertDomain(params: UpsertDomainParams) {
     ...
     */
    async ensureDomainRecord(domain: string) {
      const tld = getDomainTld(domain);

      if (!tld) {
        throw new Error(
          `Cannot persist domain "${domain}": unable to extract TLD`,
        );
      }
      ...
      const domainRecord = await this.upsertDomain({
        name: domain,
        tld,
        unicodeName,
      });
     ...
     * Returns null if the domain doesn't exist.
     */
    async getDomainNameById(id: string): Promise<{ name: string } | null> {
      const rows = await db
     ...
     */
    async updateLastAccessed(name: string): Promise<boolean> {
      try {
        const debounceThreshold = new Date(
          Date.now() - DOMAIN_UPDATE_DEBOUNCE_MS,
        );
     ...
     */
    async getRecentlyAccessedDomains(hoursAgo = 24): Promise<string[]> {
...
export type DomainsRepo = ReturnType<typeof createDomainsRepo>;
