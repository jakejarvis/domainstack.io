...
export function createFaviconsRepo(db: DbClient) {
  return {
    async upsertFavicon(params: FaviconInsert): Promise<Favicon | null> {
      const rows = await db
        .insert(favicons)
        .values(params)
        .onConflictDoUpdate({
...
export type FaviconsRepo = ReturnType<typeof createFaviconsRepo>;
