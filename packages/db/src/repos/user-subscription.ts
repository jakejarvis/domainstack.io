...
export function createUserSubscriptionRepo(db: DbClient) {
  return {
    /**
     ...
     * Update user tier.
     */
    async updateUserTier(userId: string, tier: Plan): Promise<void> {
     ...
     */
    async setSubscriptionEndsAt(userId: string, endsAt: Date): Promise<void> {
      const updated = await db
        .update(userSubscriptions)
     ...
     */
    async clearSubscriptionEndsAt(userId: string): Promise<void> {
      await db
        .update(userSubscriptions)
        .set({
          endsAt: null,
     ...
     */
    async getUserIdsWithEndingSubscriptions(): Promise<string[]> {
      const now = new Date();
     ...
     */
    async getUserWithEndingSubscription(
      userId: string,
     ...
     */
    async setLastExpiryNotification(
      userId: string,
      threshold: number,
    ): Promise<void> {
      await db
     ...
     */
    async downgradeToFree(userId: string): Promise<number> {
...
export type UserSubscriptionRepo = ReturnType<
  typeof createUserSubscriptionRepo
>;
