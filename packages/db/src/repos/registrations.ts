...
export function createRegistrationsRepo(db: DbClient) {
  return {
    async upsertRegistration(params: RegistrationInsert) {
      ...
      const normalizedNameservers: RegistrationNameserver[] = (
        nameservers ?? []
      )
    ...
    async getCachedRegistration(
      ...
      const [row] = await db
        ...
        .leftJoin(
          providers,
          eq(registrations.registrarProviderId, providers.id),
        )
...
export type RegistrationsRepo = ReturnType<typeof createRegistrationsRepo>;
