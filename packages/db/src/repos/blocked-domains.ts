...
export function createBlockedDomainsRepo(db: DbClient) {
  return {
    /**
     ...
     */
    async syncBlockedDomains(domainList: string[]): Promise<{
      added: number;
      removed: number;
      total: number;
    }> {
      ...
      const uniqueDomains = [
        ...new Set(domainList.map((d) => d.toLowerCase())),
      ];
...
export type BlockedDomainsRepo = ReturnType<typeof createBlockedDomainsRepo>;
