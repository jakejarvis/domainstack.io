...
export function createNotificationsRepo(db: DbClient) {
  return {
    /**
     ...
     */
    async updateNotificationResendId(
    ...
    async getUserNotifications(
      ...
      const [cursorNotif] = await db
        ...
        .from(notifications)
        .where(
          and(eq(notifications.id, cursor), eq(notifications.userId, userId)),
        )
        .limit(1);
     ...
     */
    async getNotificationsForTrackedDomain(
      trackedDomainId: string,
      limit?: number,
      offset = 0,
    ) {
     ...
     */
    async deleteNotificationsForTrackedDomain(
      trackedDomainId: string,
    ): Promise<boolean> {
      try {
        await db
     ...
     */
    async clearDomainExpiryNotifications(
      trackedDomainId: string,
    ): Promise<number> {
      try {
     ...
     */
    async clearCertificateExpiryNotifications(
      trackedDomainId: string,
    ): Promise<number> {
      try {
        const deleted = await db
