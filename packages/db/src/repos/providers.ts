...
export function createProvidersRepo(db: DbClient) {
  return {
    /**
     * Get provider by ID
     */
    ...
    async resolveProviderId(
      ...
      if (domain) {
        const byDomain = await db
          ...
          .orderBy(
            desc(eq(providers.source, "catalog")),
    ...
    async resolveOrCreateProviderId(
      input: ResolveProviderInput,
    ): Promise<string | null> {
      const existing = await this.resolveProviderId(input);
      ...
      try {
        ...
        if (isUniqueViolation(err)) {
          ...
          const resolved = await this.resolveProviderId(input);
     ...
     */
    async getProviderNames(ids: string[]): Promise<Map<string, string>> {
     ...
     */
    async upsertCatalogProvider(provider: Provider): Promise<ProviderRow> {
