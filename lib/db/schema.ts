...
// Domain monitoring
export const domainMonitors = pgTable(
  "domain_monitors",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: text("user_id")
  ...
  (t) => [
    unique("u_domain_monitors_user_domain").on(t.userId, t.domainId),
    index("i_domain_monitors_user").on(t.userId),
    index("i_domain_monitors_domain").on(t.domainId),
  ],
);

export const domainMonitorSettings = pgTable("domain_monitor_settings", {
  monitorId: uuid("monitor_id")
    .primaryKey()
    .references(() => domainMonitors.id, { onDelete: "cascade" }),
  notifyOnDomainExpiry: boolean("notify_domain_expiry").notNull().default(true),
  notifyOnCertExpiry: boolean("notify_cert_expiry").notNull().default(true),
  updatedAt: timestamp("updated_at", { withTimezone: true })
    .notNull()
    .defaultNow(),
});
