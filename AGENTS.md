...
- Vercel Edge Config provides dynamic, low-latency configuration without redeployment:
  ...
  - `rate_limits` (object): Service rate limits in format `{ "service": { "limit": 60, "duration": 60000 } }` (duration in milliseconds); **fails open** (no limits) if unavailable for maximum availability
...
- Rate limiting: IP-based via `@unkey/ratelimit` with service-specific namespaces; `UNKEY_ROOT_KEY` required. Durations specified in milliseconds.
